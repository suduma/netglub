...
{:name=>"powered by", 
:probability=>100,
:regexp=>/[Pp]owered by <a href="http:\/\/expressionengine.com\/">ExpressionEngine<\/a>/}
]
...
def passive
        m=[]        
        m << {:name=>"exp_last_visit cookie", :probability=>100 } if @meta["set-cookie"] =~ /exp_last_visit=/
        m
end
...
def aggressive
	...
	if body =~ /ExpressionEngine/
		m << {:name=>"readthisfirst txt file", :probability=>100}
	end
	...
	if status == 200 and body =~ /JFIF/
		m << {:name=>"ee_logo jpg", :probability=>100}
	end
