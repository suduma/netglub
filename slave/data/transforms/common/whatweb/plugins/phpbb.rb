...
matches [
{:name=>"templates/subSilver",
:probability=>100,
:regexp=>/<\!-- link rel="stylesheet" href="templates\/subSilver\/subSilver\.css/},

{:name=>"/images/logo_phpBB.gif",
:probability=>100,
:regexp=>/\/images\/logo_phpBB\.gif/},

{:name=>"Copyright notice",
:probability=>100,
:regexp=>/We request you retain the full copyright notice below including the link to www\.phpbb\.com\./},
...
def passive
	...
	m << {:name=>"phpbb2mysql_data cookie", :probability=>100, :version=>"2" } if @meta["set-cookie"] =~ /phpbb2mysql_data/i
	
	if @meta["set-cookie"] =~ /([^ ]+)_u=1; expires/
		p=@meta["set-cookie"].scan(/([^ ]+)_u=1; expires/).to_s
		m << {:name=>"phpbb3 _u, _k, _sid cookies", :probability=>100, :version=>"3" } if @meta["set-cookie"] =~ /#{p}_u=.*#{p}_k=.*#{p}_sid=[0-9a-z]{32}/
	end
...
def aggressive
	...
	unless body.nil?
		...
		if body =~ /<li><a href="[^"]+">Changes since ([0-9\.]+)<\/a><\/li>/
			...
			m << {:name=>"phpbb changelog", :probability=>100, :version=>v }
		end
	end	
	m
end
