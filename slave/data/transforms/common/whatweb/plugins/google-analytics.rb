...
def passive
	...
	if @body =~ /<script src="http:\/\/www.google-analytics.com\/urchin.js" type="text\/javascript">/
		...
		m << {:probability=>100,:name=>"urchin.js",:accounts=>accounts.sort.uniq, :string=>"urchin"}
	end
	...
	if @body =~ /var pageTracker = _gat._getTracker\("UA-([0-9]+)-([0-9]+)"\);/
		...
		m << {:probability=>100,:name=>"pageTracker = ...UA-123-1231", :string=>"GA",:accounts=>accounts.sort.uniq}
	end
	...
	if @body =~ /window.google_analytics_uacct = "UA-[0-9]+-[0-9]+";/
		...
		accounts = result.map {|x| x[0] }		
		m << {:probability=>100,:name=>"window.google_analytics_uacct = UA-123-1231",:string=>"GA", :accounts=>accounts.sort.uniq}
	end
