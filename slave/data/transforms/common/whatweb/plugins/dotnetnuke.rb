...
matches [
{:name=>"path = /portals/_default/containers or skins",
:probability=>100,
:regexp=>/src="[^"]*\/portals\/_default\/(containers|skins)/i},

{:name=>"Copyright comments",
:probability=>100,
:text=>'<!-- DotNetNuke� - http://www.dotnetnuke.com'},

{:name=>"meta copyright tag",
:probability=>100,
...
{:name=>"div tag has dnn_ctr prefix",
:probability=>100,
:text=>'<div id="dnn_ctr'},

{:name=>"div tag has dnn_ prefix",
:probability=>75,
:text=>'<div id="dnn_'},

{:name=>"__dnnVariable", # very reliable
:probability=>100,
...
#Set-Cookie: DotNetNukeAnonymous=0c88a623-73a3-4242-8506-0ea4fe21076a; expires=Fri, 09-Apr-2010 02:37:52 GMT; path=/; HttpOnly
	m << {:name=>"DotNetNukeAnonymous Cookie", :probability=>100 } if @meta["set-cookie"] =~ /DotNetNukeAnonymous=/

# passively detect modules
# modules are in the path /DesktopModules/xxx	

	if @body =~ /"\/DesktopModules\/[^\/]+/
		modules = @body.scan(/"\/DesktopModules\/([^\/]+)/).flatten.compact.sort.uniq.join(",")
		m << {:probability=>100,:name=>"modules",:string=>modules}
	end	
	
# copyright year
	if @body =~ /<!-- DotNetNuke[^<]*<!-- Copyright \(c\) 2002-[0-9]{4}/
		year = @body.scan(/<!-- DotNetNuke[^<]*<!-- Copyright \(c\) 2002-([0-9]{4})/).flatten.compact.sort.uniq.to_s
		m << {:probability=>100,:name=>"copyright year",:string=>year}
	end	
