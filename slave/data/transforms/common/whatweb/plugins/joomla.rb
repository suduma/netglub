...
matches [
{:name=>"meta generator tag 1.0",
:version=>"1.0",
:probability=>100,
:regexp=>/<meta name="Generator" content="Joomla! - Copyright \(C\) 200[0-9] - 200[0-9] Open Source Matters. All rights reserved." \/>/},

{:name=>"meta generator tag 1.5",
:probability=>100,
:version=>"1.5",
:text=>'<meta name="generator" content="Joomla! 1.5 - Open Source Content Management" />'},

{:name=>"link index.php?option=com_",
:probability=>75,
:regexp=>/<a href="[^"]*index.php\?option=com_/}
]


# extra_urls are not yet used
extra_urls ["administrator/" ]
...
#   <a href="http://www.porsche.com.br/index.php?option=com_content&task=view&id=242&Itemid=235">Galeria</a>
	...
	if @body =~ /<a href="[^"]*index.php\?option=com_/
		...
		m << {:probability=>75,:name=>"modules",:string=>modules}
	end
	
	# phpcake has this string too	
	m << {:name=>"P3P Privacy Headers", :probability=>25 } if @meta["p3p"] == "CP=\"NOI ADM DEV PSAi COM NAV OUR OTRo STP IND DEM\""	
	m
end
...
unless version.nil?
	m << {:name=>"md5sums of 4 files", :probability=>100, :version=>version}
end	
