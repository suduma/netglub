...
{:name=>"The Exchange Project",
:probability=>100,
:regexp=>/The Exchange Project - Community Made Shopping!/},
...
def passive
	m=[]
	m << {:name=>"osCsid Cookie", :probability=>100 } if @meta["set-cookie"] =~ /osCsid=[a-z0-9]{32};/
...
def aggressive
	...
	if body =~ /<img src="images\/oscommerce.png" border="0" alt="osCommerce ([^"]+)"/
		...
		m << {:name=>"admin page version", :probability=>100, :version=>version}
	end
	m
end
